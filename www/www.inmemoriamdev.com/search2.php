    ...
    if ($termes) {
        foreach ($entrees as $e) {
            ...
            foreach ($termes as $t) {
                ...
                if (mb_strpos($n_titre, $t) !== false) { $score += 12; $trouve = true; }
                if (mb_strpos($n_desc,  $t) !== false) { $score += 6;  $trouve = true; }
                if (mb_strpos($n_site,  $t) !== false) { $score += 8;  $trouve = true; }

                if ($n_texte !== '') {
                    $n = mb_substr_count($n_texte, $t);
                    if ($n > 0) { $score += min($n, 8); $trouve = true; }
                }
