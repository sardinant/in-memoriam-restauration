...
if (!file_exists($INDEX_PATH)) {
    ...
    if ($termes) {
        ...
        $freq = array();                       // freq[site][terme] = nb de pages
        ...
        foreach ($pages_par_site as $s => $n) {
            foreach ($termes as $t) {
                $part = $n > 0 && isset($freq[$s][$t]) ? $freq[$s][$t] / $n : 0;
                if ($part >= 0.75)      $poids[$s][$t] = 0.05;   // omnipresent : gabarit certain
                elseif ($part >= 0.40)  $poids[$s][$t] = 0.25;   // tres frequent : peu discriminant
                else                    $poids[$s][$t] = 1.0;
            }
        }
