...
if (!file_exists($INDEX_PATH)) {
    ...
    if ($termes) {
        ...
        // ------------------------------------------------------------
        // Poids d'un terme pour un site donne
        // ------------------------------------------------------------
        // Un mot present sur la moitie des pages d'un site ne distingue
        // rien : "jack" figure sur 49 des 58 pages de skl-network parce
        // qu'il est dans la barre de navigation. Il ne dit donc pas
        // QUELLE page consulter.
        //
        // On mesure sa frequence dans le site et on annule presque son
        // poids au-dela de 40 %. Aucun texte n'est modifie : seul le
        // classement change.

        $motifs = array();
        foreach ($termes as $t) $motifs[$t] = '/\b' . preg_quote($t, '/') . '\b/u';

        // ATTENTION : la mesure par site ne vaut que pour les gros sites.
        // Depuis que l'index est un annuaire (une entree par site ou par
        // branche), la plupart des domaines n'ont qu'une a six pages. Un
        // site d'une seule page voit TOUS ses mots a 100 % de frequence,
        // donc a poids nul : il devient introuvable. C'est ce qui faisait
        // disparaitre c-maccioni (Maggioli), jim-leroy (Nag Hammadi) et
        // les articles de Liberation (Lorski).
        //
        // On mesure donc d'abord sur le corpus entier -- un mot present
        // partout reste du gabarit -- et on n'affine par site que si le
        // site a assez de pages pour que la statistique ait un sens.

        $SEUIL_SITE = 8;                       // pages minimum par site

        $total          = count($entrees);
        $pages_par_site = array();
        $freq           = array();             // freq[site][terme]
        $freq_globale   = array();             // freq[terme] sur tout le corpus

        foreach ($entrees as $e) {
            $s = $e['site'];
            $pages_par_site[$s] = isset($pages_par_site[$s]) ? $pages_par_site[$s] + 1 : 1;
            $tout = normaliser($e['titre'] . ' ' . $e['desc'] . ' ' . $e['texte']);
            foreach ($termes as $t) {
                if (preg_match($motifs[$t], $tout)) {
                    if (!isset($freq[$s][$t]))   $freq[$s][$t] = 0;
                    if (!isset($freq_globale[$t])) $freq_globale[$t] = 0;
                    $freq[$s][$t]++;
                    $freq_globale[$t]++;
                }
            }
        ...
        $poids_global = array();
        foreach ($termes as $t) {
            $part = $total > 0 && isset($freq_globale[$t]) ? $freq_globale[$t] / $total : 0;
            if ($part >= 0.60)      $poids_global[$t] = 0.10;  // partout : gabarit
            elseif ($part >= 0.30)  $poids_global[$t] = 0.40;  // tres repandu
            else                    $poids_global[$t] = 1.0;   // discriminant
        }

        $poids = array();
        foreach ($pages_par_site as $s => $n) {
            foreach ($termes as $t) {
                if ($n < $SEUIL_SITE) {
                    // trop peu de pages : on s'en tient au corpus
                    $poids[$s][$t] = $poids_global[$t];
                    continue;
                }
                $part = isset($freq[$s][$t]) ? $freq[$s][$t] / $n : 0;
                if ($part >= 0.75)      $w = 0.05;   // omnipresent dans le site
                elseif ($part >= 0.40)  $w = 0.25;   // tres frequent
                else                    $w = 1.0;
                // on retient la penalite la plus forte des deux
                $poids[$s][$t] = min($w, $poids_global[$t]);
            }
        }
        ...
        foreach ($entrees as $e) {
            ...
            foreach ($termes as $t) {
                ...
                $m = $motifs[$t];
                $w = isset($poids[$e['site']][$t]) ? $poids[$e['site']][$t] : 1.0;
                $trouve = false;

                if (preg_match($m, $n_titre)) { $score += 12 * $w; $trouve = true; }
                if (preg_match($m, $n_desc))  { $score += 4  * $w; $trouve = true; }
                if (preg_match($m, $n_site))  { $score += 8  * $w; $trouve = true; }

                if ($n_texte !== '') {
                    $n = preg_match_all($m, $n_texte);
                    if ($n > 0) { $score += min($n, 8) * $w; $trouve = true; $dans_texte = true; }
                }
    ...
    // Contenu duplique
    // ------------------------------------------------------------
    // Deux resultats qui s'affichent a l'identique n'en font qu'un.
    // On garde celui qui score le mieux.
    $vus_signature = array();
    $uniques = array();
    foreach ($resultats as $r) {
        $sig = md5(normaliser($r['titre']) . '|' . normaliser(mb_substr($r['desc'], 0, 120)));
        if (isset($vus_signature[$sig])) {
            if ($r['score'] > $vus_signature[$sig]['score']) {
                $uniques[$vus_signature[$sig]['i']] = $r;
                $vus_signature[$sig]['score'] = $r['score'];
            }
            continue;
        }
        $vus_signature[$sig] = array('i' => count($uniques), 'score' => $r['score']);
        $uniques[] = $r;
    }
    $resultats = array_values($uniques);
